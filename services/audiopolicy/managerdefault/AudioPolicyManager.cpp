    ...
    if (usePrimaryOutputFromPolicyMixes && (preferredDevice == nullptr)) {
        ...
        if (policyDesc != nullptr) {
            ...
            *output = policyDesc->mIoHandle;
            ...
            ALOGD("getOutputForAttr() returns output %d selectedDeviceIds %s", *output, toString(*selectedDeviceIds).c_str());
    ...
    if (stream == AUDIO_STREAM_TTS) {
        *flags = AUDIO_OUTPUT_FLAG_TTS;
        ...
        if (*flags & AUDIO_OUTPUT_FLAG_FAST) {
            ...
            ALOGV("MMAP flag set, Ignoring Voip and direct output flags");
        }
        else {
...
void AudioPolicyManager::onNewAudioModulesAvailableInt(DeviceVector *newDevices)
{
    for (const auto& hwModule : mConfig->getHwModules()) {
        ...
        for (const auto& outProfile : hwModule->getOutputProfiles()) {
            ALOGV("%s: Intializing output profile(mixport): %s", __func__,
