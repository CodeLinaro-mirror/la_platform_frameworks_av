...
static const uint32_t kNormalCapturePeriodMs =
        property_get_int32("persist.audio.normal_capture_period_ms", 32);

// minimum capture buffer size for normal priority.
static const uint32_t kNormalPriorityCapturePeriodMs =
        property_get_int32("persist.audio.normal_priority_capture_period_ms", 32);

// For audio_input_flags_t flag None
static const uint32_t kMinNormalCaptureBufferSizeMs_NonLL =
        property_get_int32("persist.audio.normal_capture_nonLL_period_ms", 12);
    ...
    , mBtNrecSuspended(false)
{
    ...
    case FastCapture_Static:
        initFastCapture = !mIsMsdDevice // Disable fast capture for MSD BUS devices.
                ...
                kMinNormalCaptureBufferSizeMs_NonLL : kNormalCapturePeriodMs);
...
bool RecordThread::threadLoop()
{
    ...
    if (mType == RECORD && mSampleRate > 0 &&
            mFrameCount * MILLIS_PER_SECOND / mSampleRate < (
                mInput->flags == AUDIO_INPUT_FLAG_NONE ?
                kMinNormalCaptureBufferSizeMs_NonLL : kNormalPriorityCapturePeriodMs)) {
